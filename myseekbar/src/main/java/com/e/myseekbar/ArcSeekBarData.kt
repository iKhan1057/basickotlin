...
internal data class ArcSeekBarData(val dx: Float,
    val dy: Float,
    val width: Float,
    val height: Float,
    val progress: Int,
    ...
    val thumbX: Int =
        (r * Math.cos(alphaRad + Math.PI / 2 - progressSweepRad).toFloat() + circleCenterX).toInt()
    val thumbY: Int =
        (-r * Math.sin(alphaRad + Math.PI / 2 - progressSweepRad).toFloat() + circleCenterY).toInt()

    fun progressFromClick(x: Float, y: Float, thumbHeight: Int): Int? {
        if (y > height + dy * 2) return null
        val distToCircleCenter = Math.sqrt(
            Math.pow(
                circleCenterX - x.toDouble(),
                2.0
            ) + Math.pow(circleCenterY - y.toDouble(), 2.0)
        )
